...
func (ns *Namespace) Recover(ctx context.Context, clientRevision uint64) error {
	...
	_, span := tab.StartSpan(ctx, "sb.client.Recover")
	defer span.End()

	span.AddAttributes(
		tab.Int64Attribute("cr", int64(ns.clientRevision)),
		tab.Int64Attribute("reqcr", int64(clientRevision)))
	...
	refreshClaim := func() (time.Time, error) {
		...
		for retrier.Try(ctx) {
			expiration, lastErr = func() (time.Time, error) {
				...
				amqpClient, _, err := nsGetAMQPClientImpl(ctx)
