...
type continousSessionsArgs struct {
	numSessions          int
	rounds               int
	paddingBytes         int
...
func ContinuousSessions(remainingArgs []string) {
	...
	// testing.
	fs := flag.NewFlagSet("ContinuousSessions", flag.ContinueOnError)

	params := continousSessionsArgs{}

	fs.IntVar(&params.numSessions, "sessions", 1000000, "Number of sessions to test")
	...
	sc := shared.MustCreateStressContext("ContinuousSessions")
	defer sc.End()
